    ...
    <div className="min-h-screen" style={{background: '#080808'}}>
      ...
      <section className="relative flex items-center justify-center" style={{height: '100svh'}}>
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          ...
          <h1 className="font-serif mb-8 gold-gradient" style={{fontSize: 'clamp(3rem, 8vw, 7rem)', lineHeight: 1.05, fontWeight: 500}}>
            La Collection<br />ALMA
          </h1>
          <div className="gold-line w-24 mx-auto mb-8" />
          <p className="text-lg" style={{color: 'var(--text-muted)', letterSpacing: '0.05em', maxWidth: '36ch', margin: '0 auto'}}>
      ...
      <section className="py-28 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {watches.map((watch) => (
              <Link key={watch.id} href={`/collection/${watch.id}`} className="group block card-luxury rounded-none p-0 overflow-hidden">
                {/* Placeholder image zone */}
                <div className="relative aspect-square overflow-hidden" style={{background: '#111'}}>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <div className="font-serif text-5xl mb-2" style={{color: watch.color, opacity: 0.15}}>●</div>
                      <p className="nav-link" style={{color: watch.color, opacity: 0.4}}>{watch.id.toUpperCase()}</p>
                    </div>
                  </div>
                  <div className="absolute inset-0 flex items-end p-6">
                    <span className="nav-link" style={{color: 'var(--gold)', background: 'rgba(8,8,8,0.8)', padding: '6px 14px', backdropFilter: 'blur(8px)'}}>
                      {watch.seriesLabel}
                    </span>
                  </div>
                </div>
