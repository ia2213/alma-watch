      ...
      {hrs.map(({h,t}) => {
        ...
        return (
          <g key={h}>
            ...
            <text
              x={100+62*Math.cos(a)} y={100+62*Math.sin(a)+2.8}
    ...
    <div style={{fontFamily:"'Helvetica Neue',Arial,sans-serif", background:'#f7f7f7', minHeight:'100vh', color:'#222'}}>
      ...
      <div style={{background:KS, padding:'9px 24px', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
        <div style={{display:'flex', alignItems:'center', gap:8, fontWeight:700, fontSize:15}}>
          <svg width="20" height="20" viewBox="0 0 32 32"><circle cx="16" cy="16" r="16" fill={KS}/><path d="M10 8v16M10 16l5-8 7 8-7 8-5-8z" stroke="#000" strokeWidth="2.5" strokeLinejoin="round"/></svg>
          Kickstarter
        </div>
      ...
      <div style={{background:'linear-gradient(135deg,#0f0e0a,#1c1608 55%,#0c0b06)', padding:'64px 32px 56px'}}>
        <div style={{maxWidth:1140, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 380px', gap:64, alignItems:'center'}}>
          <div>
            ...
            <p style={{fontSize:14, color:'#999', lineHeight:1.9, maxWidth:520, marginBottom:24}}>
              La premi\u00e8re montre m\u00e9canique suisse portant les 12 scripts s\u00e9lectionn\u00e9s pour BABEL. <strong style={{color:GOLD}}>24 pi\u00e8ces limit\u00e9es</strong> num\u00e9rot\u00e9es + un reward <strong style={{color:KS}}>\u00e9dition ouverte</strong> sans limite de tirage.
            </p>
      ...
      <div style={{maxWidth:1140, margin:'0 auto', padding:'0 32px', display:'grid', gridTemplateColumns:'1fr 340px', gap:48, alignItems:'start'}}>
        ...
        <div style={{paddingTop:32}}>
          ...
          {tab === 'story' && (
            <div>
              ...
              <p style={{fontSize:15, color:'#444', lineHeight:1.9, marginBottom:14}}>ALMA vient de l&apos;aram\u00e9en et signifie <em>l&apos;\u00e9ternit\u00e9</em> ou <em>le monde</em>. BABEL rassemble une s\u00e9lection singuli\u00e8re de chiffres et signes du monde, du XII romain au \ud808\udf0b\ud808\udc79 cun\u00e9iforme.</p>
              <p style={{fontSize:15, color:'#444', lineHeight:1.9, marginBottom:28}}>La campagne propose d\u00e9sormais deux voies : les <strong style={{color:GOLD}}>24 pi\u00e8ces limit\u00e9es</strong> num\u00e9rot\u00e9es et un reward <strong style={{color:KS}}>\u00e9dition ouverte</strong>, accessible sans limite de tirage.</p>
            ...
            <div>
              ...
              <div style={{background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:24, marginBottom:28}}>
                ...
                <div style={{display:'flex', flexWrap:'wrap', gap:8, marginBottom:20}}>
                  {CIVS.map(c => (
                    <button key={c.num} onClick={() => { setNum(c.num); setReward('limited'); }} style={{width:52, height:52, borderRadius:8, border:`2px solid ${num===c.num && reward==='limited' ? c.color : '#e0e0e0'}`, background:num===c.num && reward==='limited' ? `${c.color}18` : '#fafafa', color:num===c.num && reward==='limited' ? c.color : '#888', fontSize:13, fontWeight:700, cursor:'pointer', transition:'all 0.15s', fontFamily:'Georgia,serif'}}>{c.roman}</button>
                  ))}
                </div>
                <div style={{display:'flex', gap:8, marginBottom:10}}>
                  {(['steel','gold'] as const).map(v => (
                    <button key={v} onClick={() => { setVer(v); setReward('limited'); }} style={{flex:1, padding:'10px', borderRadius:6, border:`2px solid ${reward==='limited' && ver===v ? (v==='gold' ? GOLD : KS) : '#e0e0e0'}`, background:reward==='limited' && ver===v ? (v==='gold' ? `${GOLD}12` : `${KS}10`) : '#fafafa', color:reward==='limited' && ver===v ? (v==='gold' ? '#b8860b' : '#047a4a') : '#888', fontWeight:700, fontSize:12, cursor:'pointer', letterSpacing:'0.06em'}}>
                      {v === 'gold' ? '\u2726 VERSION OR PVD \u2014 4 900 \u20ac' : '\u2699 VERSION ACIER 316L \u2014 4 500 \u20ac'}
                    </button>
                  ))}
                </div>
                <button onClick={() => setReward('open')} style={{width:'100%', padding:'12px', borderRadius:6, border:`2px solid ${reward==='open' ? KS : '#e0e0e0'}`, background:reward==='open' ? `${KS}12` : '#fafafa', color:reward==='open' ? '#047a4a' : '#666', fontWeight:800, fontSize:12, cursor:'pointer', letterSpacing:'0.08em'}}>
                ...
                <div style={{background:'#f0faf4', border:`1px solid ${KS}`, borderRadius:12, padding:28, marginBottom:24}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:16}}>
                    <div>
                      ...
                      <p style={{fontSize:14, color:'#4b4b4b', lineHeight:1.8, marginBottom:12}}>Une version sans limite de tirage, sans num\u00e9ro de pi\u00e8ce r\u00e9serv\u00e9, pens\u00e9e pour rendre BABEL plus accessible.</p>
                      <ul style={{listStyle:'none', padding:0, margin:0}}>
                        {['Design BABEL standardis\u00e9', 'Sans num\u00e9rotation individuelle I\u2013XII', 'Production non limit\u00e9e', 'Prix d\u2019acc\u00e8s Kickstarter', 'Livret de pr\u00e9sentation inclus'].map((item,i) => (
                          <li key={i} style={{fontSize:12, color:'#555', padding:'4px 0 4px 16px', position:'relative'}}><span style={{position:'absolute', left:0, color:KS, fontWeight:700}}>\u2713</span>{item}</li>
                        ))}
                      </ul>
                    </div>
                    <div style={{textAlign:'right', minWidth:130}}>
                      <div style={{fontSize:30, fontWeight:800, color:KS}}>1 900 \u20ac</div>
          ...
          {tab === 'timeline' && (
            <div>
              ...
              {TIMELINE.map((t,i) => (
                <div key={i} style={{background:'#fff', border:`1px solid ${t.cur ? GOLD : t.done ? KS : '#e8e8e8'}`, borderRadius:6, padding:'14px 18px', marginBottom:12}}>
        ...
        <aside id="pledge" style={{paddingTop:32, position:'sticky', top:64}}>
          ...
          <div style={{display:'grid', gap:8, marginBottom:14}}>
            <button onClick={() => setReward('limited')} style={{padding:'11px 12px', borderRadius:8, border:`2px solid ${reward==='limited' ? GOLD : '#e0e0e0'}`, background:reward==='limited' ? `${GOLD}12` : '#fff', fontWeight:800, fontSize:11, letterSpacing:'0.08em', cursor:'pointer', color:reward==='limited' ? '#8d6b00' : '#666'}}>
              24 PI\u00c8CES LIMIT\u00c9ES
            </button>
            <button onClick={() => setReward('open')} style={{padding:'11px 12px', borderRadius:8, border:`2px solid ${reward==='open' ? KS : '#e0e0e0'}`, background:reward==='open' ? `${KS}12` : '#fff', fontWeight:800, fontSize:11, letterSpacing:'0.08em', cursor:'pointer', color:reward==='open' ? '#047a4a' : '#666'}}>
          ...
          <div style={{background:reward==='open' ? '#f0faf4' : (ver==='gold' ? 'linear-gradient(135deg,#1c1608,#0f0c04)' : '#fff'), border:`2px solid ${reward==='open' ? KS : (ver==='gold' ? `${GOLD}50` : KS)}`, borderRadius:10, padding:18, marginBottom:14}}>
            {reward === 'limited' ? (
              ...
              <>
                ...
                <ul style={{paddingLeft:0, marginBottom:14, listStyle:'none'}}>
                  {['Design BABEL standardis\u00e9', 'Sans s\u00e9lection de num\u00e9ro I\u2013XII', 'Production ouverte', 'Livret inclus', 'Acc\u00e8s communaut\u00e9 backers'].map((item,j) => (
                ...
                <button style={{width:'100%', background:KS, color:'#000', fontWeight:700, fontSize:13, padding:12, borderRadius:5, border:'none', cursor:'pointer', letterSpacing:'0.06em'}}>
