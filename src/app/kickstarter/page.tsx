...
export default function KickstarterPage() {
  ...
  const [num,    setNum]    = useState(1); // par défaut numéro 1
        ...
        <div style={{ paddingTop:28 }}>
          ...
          {tab === 'pieces' && (
            <div>
              ...
              <p style={{ fontSize:10, color:KS, letterSpacing:'0.15em', marginBottom:20, fontWeight:700 }}>XII · ١ · ২ · 3 · ––––  · ה · ๖ · ⴦ · ፰ · Θ · 十 · 𒌋𒁹</p>
              <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:10, padding:mobile ? 16 : 24, marginBottom:24 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:mobile ? 'repeat(4, 1fr)' : 'repeat(6, 1fr)', gap:mobile ? 6 : 8, marginBottom:16 }}>
                  {CIVS.map(c => (
                    <button key={c.num} onClick={() => { setNum(c.num); setReward('limited'); }} style={{ height:mobile ? 46 : 52, borderRadius:8, border:`2px solid ${num === c.num && reward === 'limited' ? c.color : '#e0e0e0'}`, background:num === c.num && reward === 'limited' ? `${c.color}18` : '#fafafa', color:num === c.num && reward === 'limited' ? c.color : '#888', fontSize:13, fontWeight:700, cursor:'pointer', transition:'all 0.15s', fontFamily:'Georgia,serif', display:'flex', alignItems:'center', justifyContent:'center' }}>{c.roman}</button>
                  ))}
                </div>
